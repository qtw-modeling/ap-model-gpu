...
exe_files = {variants[0]: "exec_ap_cpu.exe", variants[1]: "exec_ap_gpu.exe"}


#num_launches = 1
Nxes = [16, 32, 64, 128, 256, 512]  #[int(2**n) for n in range(9, 13)] # (8, 15) --- original] # number of segments in x-dimension
#T = 100. # endtime in (ms)
...
for Nx in Nxes:
    ...
    for mode in variants:
        subprocess.call( [exe_files[mode], str(num_segments_x), str(num_segments_x),  str(tmp_output_file)] )#, str(serie_of_launches_num)] ) # Ny == Nx, for now
...
np.savetxt('speedups_%s.txt' % current_dir_name, output_2d, header='Число клеток,Время расчета CPU,Время расчета GPU,Ускорение', fmt='%.7f', delimiter=',') # 7 --- for detailed output
